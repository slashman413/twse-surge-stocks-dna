...
def compute_stock_signals(grp, target_year):
    """Screen stocks — 提高品質門檻，減少訊號但提高勝率。"""
    ...
    for i in range(200, n):
        ...
        # [D] bonus >= MIN_BONUS (3)
        is_quality = quality_score >= 40 and bonus >= MIN_BONUS
...
def process_year(year):
    ...
    # [C] 追蹤大盤連續非多頭天數
    bearish_streak = 0
    BEARISH_CLEAR_DAYS = 3  # 連續3天非多頭 → 清倉
    ...
    for date in all_dates:
        ...
        # [C] 追蹤大盤狀態
        can_trade = is_bullish and trend_up
        if can_trade:
            bearish_streak = 0
        else:
            bearish_streak += 1

        # [C] 全出清強化：大盤崩跌 或 連續非多頭達到天數 → 清倉
        should_clear = False
        clear_reason = ""
        if is_crash and positions:
            should_clear = True
            clear_reason = "大盤崩跌清倉"
        elif bearish_streak >= BEARISH_CLEAR_DAYS and positions:
            should_clear = True
            clear_reason = f"大盤連續{bearish_streak}天非多頭清倉"
        ...
        if should_clear:
            for pos in list(positions):
                ...
                trades.append({
                    ...
                    "sell_reason": clear_reason
                })
            positions.clear()
            equity_curve.append({"date": date, "equity": round(cash, 2), "type": "clear"})
            continue
